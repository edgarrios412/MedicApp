...
const SecureDownload = ({navigation}) => {
  ...
  const handleBarCodeScanned = ({ type, data }) => {
    ...
    if(type == 2048){
      ...
      // setData(datosCedula);
      const userFullName = `${user.name1} ${user.name2} ${user.lastname1} ${user.lastname2}`
      // Comparar los datos de la cedula con los de la persona
      if(datosCedula.fullName == userFullName){
        // Permite ver los resultados
        navigation.navigate("DownloadResults", datosCedula)
      }
    }else{
      // setData(null)
    }
